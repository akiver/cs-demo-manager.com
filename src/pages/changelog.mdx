...
- Incorrect round start/end ticks with some demos, resulting in invalid video sequences.
  :::warning
  It requires re-analyzing affected demos!
  :::

#### Playback

- ConCommands not working on Linux.
- Fix possible CS2 crash when the plugin reports an error on Linux.

#### Maps

- Incorrect de_cache coordinates.
- Updated de_cache radar and thumbnail images.

#### UI

- Simplified Chinese not working.
...
- Support for the 09/07/2026 CS2 update.

#### Maps

- Support for the new maps added in the 09/07/2026 CS2 update.

#### Demos

- Added [Esportligaen](https://esportligaen.dk/) demos support.
- Added support for automatically extracting demos from archives (zip, gz and bz2).
  :::warning
  This feature is disabled by default as it may slow down loading demos in folders that contain archives.  
  You can enable automatic extraction for each archive format in the **Folders** settings tab.
  :::

#### Analyze

- A team kill or suicide now doesn't decrease the player's kill count to mimic the behavior in CS2.
  :::warning
  This feature requires re-analyzing demos to have correct data!
  :::

#### Video

- Added player's color indicator in the "Player cameras" table.
- The sequence number can now be edited. It defines the order of the sequence in the final video, meaning sequences can
  be recorded in any order but will be sorted by number when generating the final video.
- Improved output filename tooltip.

#### Chat

- Chat messages can now be exported per player.
- Simplified chat messages team/players filtering logic.

#### XLSX

- Added enemies flashed and utility damage columns to players' sheets.
...
- The application now automatically uses a free WebSocket port when the default one is not available.

### Fixes

#### Playback

- Camera may not focus on the correct player with some demos.
  :::warning
  It requires re-analyzing affected demos!
  :::
- Fix possible crash on Linux when launching CS2.
- Fix CS:GO crash when generating camera thumbnail using the "Generate camera from in‑game view" button.

#### Video

- A sequence is now always generated per selected round for players.
- Player cameras being deleted when changing an existing camera target.
- POV select input being empty in the "Generate cameras" dialog.

#### Chat

- Selecting "All teams" not working in chat filter.
- Players filter possibly not applied when exporting chat messages.

#### Download

- 5EPlay demos download not working due to a recent change in their API.

#### UI

- Watch demo buttons disabled until CS is closed.
...
- New HLAE updates may not be detected, causing update button being disabled.

#### Download

- Select account input may take full width in action bar.

#### CLI

- Focus camera on player may not work when generating videos.
...
- Added 2k/3k/4k/5k columns in players' sheet.

### Fixes

#### Analyze

- Possible error since CS2 "Animgraph 2" update.

#### Video

- Dropdown menu to generate round sequence not working.

#### Matches

- Dialog when exporting to XLSX not closing at the end of the export.
...
- Support for the 14/04/2026 CS2 "Animgraph 2" update.

### Fixes

#### UI

- PRACC demos missing from dropdown filter.
- Improved accessibility of Select components.

#### Analyze

- C4 deaths now increment the death count in the scoreboard. CS2 does it since the [02/10/2025 update](https://store.steampowered.com/news/app/730/view/498333631688738077).

#### Playback

- Possible error when launching CS2 in fullscreen windowed mode.

#### Video

- HLAE version detection may fail with recent Windows builds.
...
- Added an "Analyze demos" button in the action bar to analyze demos that have not been analyzed yet.
- Added [PRACC](https://pracc.com/) demos support.

#### Match

- Avatar, bans and rank columns can now be reordered.
- Players can now be tagged directly from the scoreboard context menu.

#### Playback

- Added a "Wait for round end" option when watching player rounds.  
  When enabled, playback continues until the round ends instead of stopping at the player's death, and the camera switches to the killer 2 seconds after death.

#### Video

- Added a customizable output filename for concatenated videos.
- Added `.mov` as a supported video container.

#### UI

- Added a date format option.
- Added a timezone option for dates.

### Fixes

#### Video

- Fixed recording commands not being executed when a sequence starts, causing full demo playback instead.
- Fixed FFmpeg installation failing.

#### Analyze

- Fixed an error when a demo's path contains a comma.

#### UI

- Fixed tag buttons not being selectable through keyboard navigation.
...
- Improved loading time of the video generation screen on Windows when HLAE is installed.

### Fixes

#### Video

- Fixed possible game crash while recording videos. Thank you [@dtugend](https://github.com/dtugend) for the help!
- Fixed video may contain unwanted frames at the beginning of sequences.

#### Playback

- Game always restarting when watching a demo through HLAE when CS was already running.

#### Demos

- Fixed deduplication of demos found across multiple folders leading to selection/analysis issues.
...
- On Linux, FFmpeg is now downloaded from the latest `gpl` build of [BtbN's FFmpeg builds](https://github.com/BtbN/FFmpeg-Builds).  
  It includes more encoders than the previous build such as `h264_nvenc`.
- Moved the "Reset settings" and "Delete sequences" buttons to the 3-dot (kebab) menu.
- Improved the error message when exporting video config to JSON.

### Fixes

#### Download

- Fixed an incorrect match date after downloading demos from third-party services.
- Fixed `de_ancient` being detected as `de_warden`.

#### Video

- Fixed incompatibility with some video players and Explorer thumbnails not working.

#### Playback

- Fixed a possibly inaccurate error message when launching Counter-Strike.

#### CLI

- Added the missing `--output` flag to the `video` command help message.

#### UI

- Fixed non-Valve demos using the download date instead of the actual match date.
...
- Added a sortable column that shows when demos have been analyzed.

### Fixes

#### UI

- Fixed horizontal scrollbar may not be visible in tables.

#### Demo

- Fixed additional player named "_Deleted player_" appearing in scoreboard for non 5v5 Valve games.
...
- Added an option `--no-true-view` to disable TrueView mode when generating videos with the `video` command.

### Fixes

#### CLI

- Fixed the `--show-only-death-notices` option of the `video` command not working.
- Fixed the `--true-view` option of the `video` command not working.
...
- Added an option to launch CS in "_Fullscreen-windowed_" mode.  
  :::warning
  On Linux, the **Fullscreen** mode has been removed because Valve seems to have removed it a few months ago.  
  See [this issue](https://github.com/ValveSoftware/csgo-osx-linux/issues/4192) and [this one](https://github.com/akiver/cs-demo-manager/issues/1299).
  :::

#### Maps

- Support for the new maps added in the 21/01/2026 CS2 update.

#### Video

- Added an option to enable/disable CS2 _[TrueView](https://www.counter-strike.net/newsentry/578276333072678918)_ mode.
  :::warning
  It has been reported that when "TrueView" is enabled, recording may lags!
  :::
- The default checkboxes state is now based on existing sequences in the "_Edit sequences settings_" dialog.

#### UI

- Added Chinese Traditional. The app has been translated by [@yungchunw](https://github.com/yungchunw).
  :::info
...
- Huge memory usage when analyzing demos since the 21/01/2026 CS2 update.
- Fixed error during analysis when the Windows username contains a single quote.

#### Playback

- Playback always starting from the beginning on **Linux** since the 21/01/2026 CS2 update.
- **CS:GO** not launching on **Linux** when the CS2 Steam runtime script location option is enabled.

#### Settings

- Button to reveal the **CS:GO** log file not working on Linux.

#### Video

- Players' death notices visibility settings may not work correctly.
- Last player may not be visible in the table from the "_Edit sequences settings_" dialog.

#### UI

- Fixed wrong demo/match date timezone.
...
- Added "**Weapon inspections**" panel in match and player's global page that shows how many time a player inspected
  his weapon and how many times he died while inspecting it.
  :::warning
  This feature requires re-analyzing demos to have correct data!
  :::
- Added `weapon_inspection_count` and `deaths_while_inspecting_weapon` columns in Excel exports.

### Fixes

#### Video

- Death notices may not appear when sequences overlap.

#### CLI

- Video command help shown instead of starting video generation when `--config-file` is the only arg defined.
- Video recording not starting when using relative demo paths.
- Video sequences defined in JSON config file were overridden resulting in a single broken sequence.

#### UI

- Increased the maximum width of the first column in the sequence edition dialog.
...
- Heatmaps are now zoomable and pannable.
- [Comments](/docs/guides/comments) can now be added to rounds.
- `de_nuke` radar images have been updated.

#### Download

- Added Renown support for third-party demo downloads.
- Increased the number of matches fetched to 20 when retrieving lists from third-party services.
- Improved the download settings UI.

#### Video

- Added [fixed camera](/docs/guides/cameras) positions.
- Added a button to only watch sequences without generating videos.
- Added an option to toggle audio recording.
- Added buttons to copy/export the current video configuration to JSON.
- CS2 TrueView telemetry is now hidden using the command `cl_trueview_show_status`.
- Improved camera focus for overlapping kill sequences.

#### CLI

- Added a [command](/docs/cli#download-renown-demos) to download Renown demos.
- Added a [command](/docs/cli#download-5eplay-demos) to download 5EPlay demos.
- Added `--mode=player` and `--event` arguments to the [`video` command](/docs/cli#generate-videos) to generate players' sequences.
- Added `--config-file` argument to the [`video` command](/docs/cli#generate-videos) to load configuration from a JSON file.

#### Search

- Replaced kill related events with a more flexible kills filter that allows combining multiple conditions (headshot, through smoke, no scope, etc.).

#### Player

- Added a tab to generate heatmaps for a specific player.

#### Team

- Added an input to filter on players in the heatmap view.

#### Playback

- Added a workaround to start demo playback when Steam is offline.

#### Bans

- Format numbers based on the user's locale in ban-related views.

#### Database

- Increased database connection timeout to improve stability on slow connections.

### Fixes

#### Playback

- Fixed possible crash when starting demo playback.

#### Analyze

- Fixed possible error during analysis.
...
- The new HLAE command `mirv_panorama` is now automatically applied to hide "TrueView" telemetry text.

### Fixes

#### Video

- Recording may randomly not start.
